<#
    ...
    Modules/M365DSC.mgx/en-US/Mgx.Cmdlets.dll-Help.xml is generated from Modules/M365DSC.mgx/help/*.md, and for two
...
BeforeAll {
    ...
    $script:Maml = Join-Path $repo 'Modules/M365DSC.mgx/en-US/Mgx.Cmdlets.dll-Help.xml'
    $script:HelpDir = Join-Path $repo 'Modules/M365DSC.mgx/help'
...
Describe 'Compiled help matches its markdown source' {
    ...
    It 'carries no description the markdown has replaced' {
        ...
        ($hits | ForEach-Object { "$($_.Text) -- $($_.Why)" }) -join '; ' |
            Should -BeNullOrEmpty -Because 'the MAML is generated from Modules/M365DSC.mgx/help; run ./build.ps1 to regenerate it'
    }
...
Describe 'Documented output types match the cmdlets' {
    ...
    It 'every cmdlet documents the type it declares' {
        ...
        foreach ($cmd in Get-Command -Module M365DSC.mgx -CommandType Cmdlet) {
            $declared = @($cmd.OutputType | ForEach-Object { $_.Type.FullName } | Where-Object { $_ })
            if (-not $declared) { continue }

            $md = Join-Path $repo "Modules/M365DSC.mgx/help/$($cmd.Name).md"
        ...
        ($wrong -join "`n") | Should -BeNullOrEmpty -Because 'Modules/M365DSC.mgx/help OUTPUTS should name the type the cmdlet emits'
    }
}
