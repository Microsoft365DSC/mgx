...
Describe 'Help content is authored, not auto-generated' {
    ...
    BeforeAll {
        ...
        $script:HelpMd   = Join-Path $script:Paths.ModuleRoot 'help'
    ...
    It 'compiled help carries every markdown example for <_>' -ForEach $script:ExpectedCmdlets {
        ...
        $inMaml | Should -Be $inMarkdown -Because "help/$name.md documents $inMarkdown example(s) but the MAML carries $inMaml - regenerate the compiled help"
    }
