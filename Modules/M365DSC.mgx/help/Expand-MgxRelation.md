...
Module Name: Mgx
online version: https://github.com/Microsoft365DSC/mgx/blob/main/Modules/M365DSC.mgx/help/Expand-MgxRelation.md
schema: 2.0.0
---
...
The /manager endpoint returns a single object, not a collection. -Flatten unwraps it from the array so Manager is a Hashtable (not a one-element array). Users without a manager get $null.
...
### -InputObject
The object to enrich with related data. Accepts a Hashtable (such as the output of another Mgx cmdlet) or a PSCustomObject from the pipeline. Must have a member matching -IdProperty (default: id) whose value is substituted into the -Uri template. The relation is attached in the same shape the object arrived in.
...
### -Flatten
Unwrap single-value relations. When the endpoint returns exactly one item (or a singleton object like /manager), the result is attached as a single Hashtable instead of a one-element array. If multiple items are returned, a warning is emitted and the array is returned as-is.
...
The input object with an additional member (named by -As) containing the relation data. Collection endpoints produce an array of Hashtables. Singleton endpoints produce a single Hashtable when -Flatten is used, or a one-element array otherwise. A PSCustomObject piped in is returned as a PSCustomObject.
